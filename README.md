...
상세 페이지 위치 섹션은 Google Street View Static API를 우선 사용하고, 키가 없거나 스트릿뷰가 없으면 기존 이미지로 폴백합니다. 좌표가 있는 항목은 좌표 기반으로 더 정확하게 맞춥니다.

페이지별 로딩, 에러, 빈 상태는 `PageState` 공통 컴포넌트로 정리해 유지보수성을 높였습니다.
